...
This repo now tracks the Yahoo MCP as a **backend application** that is intended to sit behind a shared HTTPS edge on ports 80 and 443, alongside other MCP services.
...
- move from mock mode to live Yahoo mode when the account allows it
...
- keep `MAIL_MODE=mock` until Yahoo app-password generation is actually available
...
2. align the GitHub MCP repo to the same backend-only pattern
3. run both services behind the shared `mcp-infra` edge on ports 80 and 443
4. switch Yahoo MCP to live mode once Yahoo app-password support is available

That is the shape of the work now. The Yahoo MCP is no longer the special-case public edge. It is one backend in a shared MCP platform model.
